# Passer KLEAN'STOR sur `kleanstor.com`

Objectif : `app.kleanstor.com` (frontend, Cloudflare Worker) et `api.kleanstor.com`
...
2. Chercher `kleanstor.com`, l'acheter (~12 $/an, au prix coûtant, sans majoration)
...
`kleanstor.com` → copier les 2 nameservers Cloudflare chez le registrar. Attendre
la propagation.)*

---

## 2. `api.kleanstor.com` → Render

### 2a. Render
1. Render → service **cleanstore-backend** → **Settings → Custom Domains → Add**
2. Saisir `api.kleanstor.com`
3. Render affiche une cible **CNAME** (du genre `cleanstore-backend.onrender.com`).

*(Le `render.yaml` déclare déjà `domains: [api.kleanstor.com]` ; l'ajouter à la
main garantit juste que c'est pris en compte tout de suite.)*

### 2b. Cloudflare DNS
1. Cloudflare → zone `kleanstor.com` → **DNS → Records → Add record**
...
Vérif : `https://api.kleanstor.com/api/health` doit répondre `{"ok":true}`.

---

## 3. `app.kleanstor.com` → Cloudflare Worker

1. Cloudflare → **Workers & Pages → cleanstore → Settings → Domains & Routes → Add → Custom Domain**
2. Saisir `app.kleanstor.com` → Add. Cloudflare crée l'enregistrement DNS et le
...
Vérif : `https://app.kleanstor.com` charge l'app (elle parlera encore à l'ancienne
...
- `FRONTEND_URL` = `https://app.kleanstor.com,https://cleanstore1.cephbookman.workers.dev`
...
- Plus tard, une fois `app.kleanstor.com` validé partout : enlever l'ancienne URL.
...
2. Éditer `frontend/.env.production` :
   `VITE_API_URL=https://api.kleanstor.com/api`
...
3. Ouvrir `https://app.kleanstor.com`, se connecter, cliquer partout. Le cookie de
...
1. Resend → **Domains → Add Domain** → `kleanstor.com`
...
4. Render → env :
   ...
   - `RESEND_FROM_EMAIL` = `KLEAN'STOR <no-reply@kleanstor.com>`
...
`https://app.kleanstor.com/wallet` une fois l'étape 4 faite). Juste revérifier
...
- Render `FRONTEND_URL` : enlever l'ancienne URL `workers.dev`, garder seulement
  `https://app.kleanstor.com`. Redéployer.
- Optionnel : sur l'ancien domaine `cleanstore1.cephbookman.workers.dev`, mettre
  une redirection 301 vers `app.kleanstor.com`.
...
- `frontend/.env.production` : nouveau fichier, `VITE_API_URL` (sur l'URL Render
  actuelle ; à basculer sur `api.kleanstor.com` au moment voulu).
- `render.yaml` : `domains: [api.kleanstor.com]`.
