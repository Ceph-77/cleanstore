...
import { buildIncidentReport, buildMonthlyStatement, buildWithdrawalReceipt } from "./documents.service";
...
export async function incidentReport(req: Request, res: Response) {
  if (!can(sessionRoles(req), "feedback", "view")) {
    return res.status(403).json({ error: "Forbidden" });
  }
  try {
    const pdf = await buildIncidentReport(req.params.id);
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `inline; filename="incident-${req.params.id.slice(0, 8)}.pdf"`);
    res.send(pdf);
  } catch (err) {
    res.status(400).json({ error: (err as Error).message });
  }
