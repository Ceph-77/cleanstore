...
import { claimDecisionSchema } from "./storeClaims.schema";
...
export async function create(req: Request, res: Response) {
  try {
    const claim = await storeClaimsService.createClaim(req.params.storeId, req.session.userId!);
...
export async function decide(req: Request, res: Response) {
  ...
  const claim = await storeClaimsService.decideClaim(req.params.id, parsed.data.status);
  res.json({ claim });
}
